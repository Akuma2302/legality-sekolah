...
export default function RandomDetailModal({ entry, onClose, onSaved }) {
  const [form, setForm] = useState({
    school_name: entry.school_name || '',
    pic_name: entry.pic_name || '',
    type: entry.type || SCHOOL_TYPES[0],
    ...
    program_propose: entry.program_propose || '',
    note: entry.note || '',
  });
  const [saving, setSaving] = useState(false);
  const [savedAt, setSavedAt] = useState(entry.updated_at);
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const updated = await api.updateRandomEntry(entry.id, form);
  ...
  return (
    <Modal title={entry.school_name} onClose={onClose} wide>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-3">
          <div className="flex-1">
            <Field label="Status" hint="Tracks progress on this entry.">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full sm:w-auto sm:mt-6 bg-navy-900 text-white rounded-lg px-5 py-2 text-sm font-medium hover:bg-navy-800 disabled:opacity-50 whitespace-nowrap"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
        ...
        <TeacherCRM parentType="random" parentId={entry.id} />
        <MomNoteSection parentType="random" parentId={entry.id} />
